...
interface ICreteTask {
  title: string;
}

export const ModalCreateTask = () => {
  const { isLoading, CreateTask } = useContext(TaskContext);
  const { isOpenCreateTask, onCloseCreateTask } = useContext(ContextModal);
  ...
  const onSubmit = (data: ICreteTask) => {
    CreateTask(data);
    onCloseCreateTask();
    reset();
  };
        ...
        >
          <div className="m-auto text-xl">
            <ModalHeader className="text-green-100">
              <h2 className="text-xl tablet:text-2xl">Criar tarefa</h2>
            </ModalHeader>
